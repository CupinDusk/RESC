...
for(int r = 0; r < repeat; r++){
  ...
  if (rank == 0) {
    ...
    //if (tid == 0) {
      //printf("\n[WRITE] rank=%d, smid=%u, wrote value=%.1f\n", rank, get_smid(), (double)write_value);
   //}
  } else {
    // 其他 SM：进行全局读
    //acc = ld_ca_f32(addr);  // 通过 L1D；若读共享生效，可从 peer L1 提供
  //  if (tid == 0) {
  //    printf("\n[READ] rank=%d, smid=%u, read value=%.1f\n", rank, get_smid(), (double)acc);
  //  }
  }
