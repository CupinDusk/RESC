  ...
  for (int ks=0; ks<num_k_slices; ++ks) {
    ...
    if (i_am_producer) {
      for (int i=threadIdx.x;i<TILE_M*TILE_K;i+=blockDim.x){
        int r=i/TILE_K, c=i%TILE_K; A_s[i] = A[idx2(row0 + r, k_base + c, ldA)];
      }
      for (int i=threadIdx.x;i<TILE_K*TILE_N;i+=blockDim.x){
        int r=i/TILE_N, c=i%TILE_N; B_s[i] = B[idx2(k_base + r, col0 + c, ldB)];
      }
      __threadfence_block();
      // 无 DSM：拷到 GMEM tile 缓冲
...
int main(int argc, char** argv){
  ...
  cudaMalloc(&d_A_tile, TILE_M*TILE_K*sizeof(float));          // 64*32*4B = 8KB
  cudaMalloc(&d_B_tile, TILE_K*TILE_N*sizeof(float));          // 32*64*4B = 8KB
